...
export default function AboutPage() {
  ...
  const { setting: aboutSetting } = useSiteSetting('site', 'about_image');
  const aboutImageSrc = useMemo(() => {
    const raw = aboutSetting?.image || aboutSetting?.link || aboutSetting?.value;
    if (raw && String(raw).trim()) return fixImageUrl(String(raw).trim());
    return '';
  }, [aboutSetting]);
  ...
  return (
    <div className={`min-h-screen bg-bgSection ${isRTL ? 'rtl' : 'ltr'}`}>
      ...
      <section className="py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            <div className="bg-bgSection rounded-2xl p-8">
      ...
      <section className="py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-3xl md:text-4xl font-bold text-textPrimary text-center mb-12">
